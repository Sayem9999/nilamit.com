...
export default function ProfilePage() {
  ...
  const handleSaveStorefront = () => {
    startTransition(async () => {
      ...
      if (res.success) {
        await update();
        toast.success("Storefront biography updated!");
      } else {
        toast.error(res.error?.message || "Failed to update biography");
      }
    });
  };
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div 
                    ...
                    className="group relative w-full h-32 md:h-44 rounded-2xl bg-gray-50 border border-dashed border-gray-300 overflow-hidden flex items-center justify-center cursor-pointer transition-all hover:border-primary-500"
                  >
                    <input
                      type="file"
                    ...
                    {session.user?.banner ? (
                      ...
                      <div className="absolute inset-0 bg-gradient-to-br from-primary-500 to-indigo-700 opacity-90 flex flex-col items-center justify-center p-4 text-center text-white">
                        <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center mb-2">
                          <Camera size={20} className="text-white" />
                        </div>
                        <span className="text-sm font-black uppercase tracking-widest text-white">No Banner Uploaded</span>
                        <span className="text-[10px] text-white/80 mt-1">Click to upload custom cover photo (1200 x 400 suggested)</span>
                      </div>
                    )}

                    {/* Change photo hover overlay */}
                    {!isUploadingBanner && !isRemovingBanner && session.user?.banner && (
                      <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-1 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <div>
                  <label htmlFor="storefront-bio" className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">
                    Business Biography / Store Description
                  </label>
                  <p className="text-xs text-gray-500 mb-3">
                    Introduce your store to buyers. Share your listing categories, shipping methods, and brand mission.
                  </p>
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      placeholder="Welcome to our store! We sell authentic collectibles and vintage items in Bangladesh. Fast shipping and 100% trust guaranteed..."
                      className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm font-bold text-gray-900 outline-none focus:ring-2 focus:ring-primary-500 resize-none"
                    />
                    <div className="absolute bottom-3 right-3 text-[10px] font-black text-gray-400 tracking-wider">
                      {bio.length} / 1000
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={isPending}
                    className="flex items-center gap-2 px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white font-black text-xs uppercase tracking-widest rounded-[1.5rem] transition-all shadow-md hover:shadow-primary-500/10 active:scale-95 disabled:opacity-50"
                  >
                    {isPending ? (
