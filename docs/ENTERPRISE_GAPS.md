...
| **Live share cards** | **Shipped** | `GET /api/share-card/{auctionId}` — live 1200×630 PNG (current bid + countdown; green SOLD result variant) for the FB/WhatsApp posting loop in `docs/GROWTH.md`. In the auction Share menu as "Download share card". |
| **Cold-start landing page** | **Shipped** | Homepage adapts to its own liquidity: how-it-works/escrow story renders above the fold when no auction rails exist; StatsBar gated until ≥100 bids/users. |
| **Phone OTP verification (Firebase)** | **Console-gated** | Full flow shipped: `PhoneVerificationCard` (dashboard settings) runs Firebase Phone Auth (invisible reCAPTCHA → SMS → 6-digit code, linked to the session-bound Firebase user), `confirmPhoneVerification` verifies the ID token server-side (UID binding + per-number uniqueness) and sets `users/{uid}.isPhoneVerified` + `phoneNumber`. **Activate:** Firebase Console → Authentication → Sign-in method → enable **Phone** (requires Blaze; SMS billed per message, BD supported). Until enabled the card shows a clean "not enabled yet" message. CSP already whitelists reCAPTCHA/gstatic. No app OTP storage — Firebase owns code generation/expiry (rule 5 by construction). |
