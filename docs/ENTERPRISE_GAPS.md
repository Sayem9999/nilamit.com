...
| 6 | Featured-listing self-serve purchase | **scaffolded** | `Auction.featuredUntil` + `Server Actions` (`quoteFeaturedPurchase`, `activateFeatured`) shipped. Need: seller-facing UI on auction detail + payment-callback routing for `feat-*` tran IDs + hourly expiry cron. |
...
| 8 | Seller KYC | **scaffolded** | `User.kycStatus` + `kycDocsRef` fields + Server Actions (`submitKyc`, `approveKyc`, `rejectKyc`, `listPendingKyc`). Need: seller UI form in retailer settings + admin moderator queue tab. |
